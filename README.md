...
1. Obsidian에서 **BRAT** 플러그인을 설치합니다 (설정 → 커뮤니티 플러그인 → BRAT)
2. `Cmd+Shift+P` → **BRAT: Add a beta plugin for testing** 선택
3. 다음 URL을 입력합니다:
   ```
   https://github.com/NoahRha/obsidian-content-engine
   ```
4. 설정 → BRAT 플러그인 → **Check for updates and install** 클릭
5. 설치 후 **Content Engine** 플러그인을 활성화합니다
...
4. **분석 시작**: "분석 시작" 버튼 클릭
5. **콘텐츠 생성**: 각 탭(블로그/유튜브/Thread/인스타그램/Twitter/GitHub)에서 "재생성" 버튼으로 콘텐츠 생성
...
- 📱 **소셜 미디어**: Thread, Instagram, YouTube, Twitter/X, GitHub 동시 발행
- ✅ **연결 테스트**: 각 플랫폼별 API 연결 상태 실시간 테스트
- 📝 **커스텀 템플릿**: 플랫폼별 템플릿 편집 기능
...
- **LLM 모델**: 사용할 AI 모델 선택 (GPT-4, Claude, DeepSeek 등)
- **상세 모델명**: "직접 입력" 선택 시 상세 모델명 직접 입력 (예: anthropic/claude-sonnet-4-6)
- **연결 테스트**: API 서버 연결 상태 테스트 버튼
- **플랫폼 토글**: 사용할 플랫폼 선택 (Instagram, Threads, YouTube, Twitter/X, GitHub)
- **플랫폼 연결 테스트**: 각 플랫폼별 API 연결 상태 개별 테스트 버튼
- **게시 템플릿**: 플랫폼별 템플릿 편집 ({title}, {summary}, {content}, {description}, {hashtags}, {tags} 변수 지원)
